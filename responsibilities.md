...
|Pulling deployable architecture changes into a project| {{site.data.keyword.IBM_notm}} provides the ability for customers to update the version of a deployable architecture in a project if a new version becomes available. | Customers are notified when a [new deployable architecture version](docs/secure-enterprise?topic=secure-enterprise-needs-attention-projects#na-version-update) is available so they can update their project.
