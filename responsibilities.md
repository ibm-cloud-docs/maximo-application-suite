...
|Meet high availability objectives| {{site.data.keyword.cloud}} is available globally and load balanced from a single URL. It is highly available and continues to run even if your resources are unavailable. \n For more information about high availability, see the [{{site.data.keyword.IBM}} service level objectives](/docs/overview?topic=overview-slo) and the [sample application architecture](/docs/overview?topic=overview-bcdr-app-recovery).  | N/A |
{: row-headers}
